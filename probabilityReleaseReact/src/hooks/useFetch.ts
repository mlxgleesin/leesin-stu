...
/**
 * 支持统一的umi-request参数；
 * 但是为了兼容原有的调用情况，method为post时，params被当做data参数，当data和params同时存在时，params参数生效；
 */
...
/** 后置处理函数，对接口返回值进行处理，并把处理结果存放在useState中，避免过多计算，对只处理一次的接口有很好的优化效果 */
